...
row_details <- function(index) {
  ...
  if (!is.na(isbn)) {
    isbn <- htmltools::tags$a(href = paste0("https://isbndb.com/book/", isbn), target = "_blank", isbn)
  }

  english_link <- lore[["Link (englisch)"]][index]
  if (!is.na(english_link)) {
    english_link <- htmltools::tags$a(href = english_link, target = "_blank", english_title)
  }
...
lore_reactable <- function(lore) {
  reactable::reactable(
    dplyr::select(lore, Status, Kosten, Titel),
    resizable = TRUE,
    compact = TRUE,
    striped = TRUE,
    columns = list(
...
lore_data <- function(sheet = 1) {
  ...
  readxl::read_excel(path, sheet) |>
    ...
    dplyr::mutate(
      #pic_full_path = fs::path(getwd(), pic),
